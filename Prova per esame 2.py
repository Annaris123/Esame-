...
def leggi_dati_network_nodes(network_nodes):
    ...
    for riga in file[1:]:  # Salta la prima riga
        ...
        if luogo not in luogo_servizi:  # Se il luogo non è ancora nel dizionario, lo aggiungiamo
            luogo_servizi[luogo] = [fermata_id]
        elif fermata_id not in luogo_servizi[luogo]:  # Se il luogo esiste già, ma il servizio no, lo aggiungiamo
            luogo_servizi[luogo].append(fermata_id)
